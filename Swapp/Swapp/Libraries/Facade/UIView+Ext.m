...
- (void)groupVertically:(NSArray *)subviews topCenterWithTopPadding:(CGFloat)top spacing:(CGFloat)spacing height:(CGFloat)height {
    CGFloat yOrigin = top;
    
    for (UIView *view in subviews) {
        view.frame = CGRectMake((CGRectGetWidth(self.frame) / 2.0) - (view.width / 2.0), yOrigin, view.width, height);
